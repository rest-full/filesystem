    ...
    public function valid(string $type): bool
    {
        if ($type == 'video') {
            if (!in_array(
                ...
                array_keys($this->videoMime)
            )
            ) {
                throw new Exceptions(
                    "This video has no accepted extension. Extensions accepted: MP4, WebM.",
                    404
                );
            }
            return true;
            ...
            if (!in_array(
                mime_content_type($this->file),
                array_keys($this->audioMime)
            )
            ) {
                throw new Exceptions(
                    "This audio has no accepted extension. Extensions accepted: MP3, WebM.",
                    404
                );
            }
            return true;
