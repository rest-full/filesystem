...
class Image extends File
{
     ...
     */
    public function __construct(string $file, array $arq = null)
    {
        if (isset($arq)) {
            parent::__construct($file, $arq);
            return $this;
        }
    ...
    {
        if ($type == 'Image') {
            $aa = array_keys($this->imageMime);
            $ab = $this->tmp('type');
            $ac = in_array($ab, $aa);
            if (in_array($this->tmp('type'), array_keys($this->imageMime))
                === false
            ) {
    ...
    public function createDifferentSizes(string $sizes, string $imageTmp): Image
    {
        $this->resize(
            $this->calculating(
                substr($sizes, 0, stripos($sizes, 'x')),
                substr($sizes, stripos($sizes, 'x') + 1)
            ),
            [$imageTmp],
            'ico'
        );
